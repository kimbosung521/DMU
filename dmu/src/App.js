...
function App() {
  ...
  const handleSelectAnswer = (selected) => {
    ...
    setUserAnswers(prev => [...prev, { question: q.question, correct: isCorrect }]);
  };
    ...
    return (
      <>
        <div className="quiz-header">
          <span className="quiz-count"><strong>{currentIndex + 1}</strong> / {currentQuestions.length}</span>
          <span className={`quiz-type-badge ${q.type === 'mc' ? 'badge-mc' : 'badge-ox'}`}>
            {q.type === 'mc' ? '사지선다' : 'O / X'}
          </span>
        </div>
        ...
        {q.type === 'mc' ? (
          <div className="options-grid mc">
            {q.options.map((opt, i) => {
              ...
              return (
                <button key={i} className={btnClass} onClick={() => handleSelectAnswer(i)} disabled={answered}>
                  {opt}
                </button>
              );
            })}
          ...
          <div className="options-grid ox">
            {['O', 'X'].map((label, i) => {
              ...
              return (
                <button key={i} className={btnClass} onClick={() => handleSelectAnswer(val)} disabled={answered}>
                  {label}
                </button>
              );
            })}
        ...
        {answered && (
          <div id="feedback">
            <div className={`feedback-box ${isCorrect ? 'correct-fb' : 'wrong-fb'}`}>
              {isCorrect ? '✅ 정답입니다!' : '❌ 오답입니다!'}<br />{q.explanation}
            </div>
  ...
  const renderResult = () => (
    <>
      <div className="result-score-ring" style={{ background: '#f0fdf4', border: '4px solid #22c55e', color: '#166534' }}>
        <div className="score-num">{score}</div>
        <div className="score-denom">/ {QUESTIONS_PER_ROUND} 맞음</div>
      </div>
      <div className="result-title">수고하셨습니다!</div>
      <div className="result-msg">취업지원센터 부스로 오셔서<br />결과 화면을 보여주세요 🎁</div>
      <button className="btn-retry" onClick={() => setGameState('START')}>처음으로 돌아가기</button>
    </>
  );
  ...
  return (
    <div style={{
      ...
      <div className="total-counter" style={{ marginTop: '20px', color: 'rgba(255,255,255,0.7)', fontSize: '14px', fontWeight: 'bold' }}>
        {participantsCount > 0 ? `오늘 총 ${participantsCount.toLocaleString()}명 참여 완료! 🎉` : ''}
      </div>
